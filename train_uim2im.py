...
def main_train_imageEncoder():
    ...
    if is_weighted_loss:
        if is_stackGAN:
            net_d_name = os.path.join(save_dir, 'net_stackD.npz')
        else:
            net_d_name = os.path.join(save_dir, 'net_d.npz')
        if not os.path.exists(net_d_name):
            print("[!] Loading D or D II checkpoint failed!")
        else:
    ...
    for step in range(n_step):
        ...
        # b_caption = captions_ids_test[idexs]
        # b_caption = tl.prepro.pad_sequences(b_caption, padding='post')
        # im = images_test[np.floor(np.asarray(idexs).astype('float')/n_captions_per_image).astype('int')]
        # im = threading_data(im, prepro_img, mode='translation')
        # print(im.shape)
        # p = sess.run(net_d.outputs, feed_dict={ # debug
        #                             t_caption : b_caption,
        #                             t_image : im,
        #                                 })
...
def main_train_rnn2cnn():
    t_image = tf.placeholder('float32', [batch_size, image_size, image_size, 3], name = 'input_image')
    t_caption = tf.placeholder(dtype=tf.int64, shape=[batch_size, None], name='input_caption')

    net_rnn = rnn_embed(t_caption, is_train=False, reuse=False, return_embed=False)
    net_c = cnn_encoder(t_image, is_train=False, reuse=False, name='rnn2cnn')

    loss =  tf.reduce_mean( tf.square( tf.sub( net_rnn.outputs, net_c.outputs) ))
    ...
    train_vars = tl.layers.get_variables_with_name('rnn2cnn', True, True)
    ...
    for epoch in range(0, n_epoch+1):
        ...
        for step in range(n_batch_epoch):
            ...
            err, _ = sess.run([loss, train_op], feed_dict={
                                t_caption : b_real_caption,
                                t_image : b_real_images})
...
def main_translation_2images():
    ...
    t_image = tf.placeholder('float32', [batch_size, image_size, image_size, 3], name = 'input_image')
    t_image_2 = tf.placeholder('float32', [batch_size, image_size, image_size, 3], name = 'input_image_2')
    # t_caption = tf.placeholder(dtype=tf.int64, shape=[batch_size, None], name='input_caption')

    net_p = cnn_encoder(t_image, is_train=False, reuse=False, name="image_encoder")
    # tl.layers.set_name_reuse(True)
    net_c = cnn_encoder(t_image_2, is_train=False, reuse=False, name='rnn2cnn')
    # net_rnn = rnn_embed(t_caption, is_train=False, reuse=False, return_embed=False)
    # print(net_c.outputs, net_rnn.outputs)
    # exit()
    net_g, _ = generator_txt2img(net_p.outputs, # image --> image
                    net_c, is_train=False, reuse=False)
    net_g.print_layers()
    # exit()

    if is_stackGAN:
        net_gII, _ = stackG(net_g.outputs, net_c, is_train=False, reuse=False)
    ...
    # os.system("mkdir checkpoint/step3")
    os.system("mkdir samples/step3")
    net_c_name = os.path.join(save_dir, 'net_rnn2cnn.npz')
    net_g_name = os.path.join(save_dir, 'net_g.npz')
    net_p_name = os.path.join(save_dir, 'net_p.npz')

    # load generator, RNN and Encoder
    net_c_loaded_params = tl.files.load_npz(name=net_c_name)
    tl.files.assign_params(sess, net_c_loaded_params, net_c)
    net_g_loaded_params = tl.files.load_npz(name=net_g_name)
    # tl.files.assign_params(sess, net_g_loaded_params, net_g)
    # for p in net_g_loaded_params:
    #     print(p.shape)
    # net_g.print_params(False)
    ops = []
    for idx, param in enumerate(net_g_loaded_params[3:]):
        ops.append(net_g.all_params[idx+18].assign(param))
    sess.run(ops)
    net_p_loaded_params = tl.files.load_npz(name=net_p_name)
    tl.files.assign_params(sess, net_p_loaded_params, net_p)
    if is_stackGAN:
    ...
    for i in range(1):
        ...
        # print(np.min(gen_img), np.max(gen_img))
        save_images(b_images, [8, 8], 'samples/step3/source_{:02d}.png'.format(i))
        save_images(b_images_2, [8, 8], 'samples/step3/source2_{:02d}.png'.format(i))
        save_images(gen_img, [8, 8], 'samples/step3/translate_{:02d}.png'.format(i))
        # print("Translate completed {}".format(i))
...
if __name__ == '__main__':
    ...
    elif args.train_step == "train_rnn2cnn":
        main_train_rnn2cnn()
