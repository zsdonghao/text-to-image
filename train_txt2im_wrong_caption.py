...
def prepro_img(x, mode=None):
    ...
    # flip : https://github.com/paarthneekhara/text-to-image/blob/master/Utils/image_processing.py
        ...
        x = rotation(x, rg=20, is_random=True, fill_mode='nearest')
        x = crop(x, wrg=55, hrg=55, is_random=True)
    ...
    # rescale (-1, 1) --> (0, 1) for display
        ...
        x = rotation(x, rg=20, is_random=False, fill_mode='nearest')
        x = crop(x, wrg=55, hrg=55, is_random=True)
        x = imresize(x, size=[64, 64], interp='bilinear', mode=None)
        x = x / 255.
    else:
...
# # pre_img = prepro_image_array(images[:64])
...
d_loss2 = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(disc_wrong_caption_logits, tf.zeros_like(disc_wrong_caption_logits)))  # remove if DCGAN only
...
# no pre-trained RNN, update RNN only in D:
#   size=512 dp=0.5         g_loss increase after 700 epochs, D RNN overfit ?     that is why im2txt2im is better?
...
for epoch in range(n_epoch):
    ...
    for step in range(n_batch_epoch):
        ...
        errD, _ = sess.run([d_loss, d_optim], feed_dict={
                        ...
                        t_wrong_caption : b_wrong_caption,  # remove if DCGAN only
